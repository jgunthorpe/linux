...
with open("drivers/vfio/vfio.c", "rt") as F:
    g = re.search(
        r"static const u8 vfio_from_fsm_table\[VFIO_DEVICE_NUM_STATES\]\[VFIO_DEVICE_NUM_STATES\] = {(.*?)};",
...
# Enable to calculate when PRE_COPY is not supported
if False:
    for c, n in list(all_arcs.keys()):
        if (c == "PRE_COPY" or c == "PRE_COPY_P2P" or n == "PRE_COPY"
                or n == "PRE_COPY_P2P"):
            del all_arcs[c, n]
    for c, n in list(all_arcs.keys()):
        s = all_arcs[c, n]
        assert s != "PRE_COPY" and s != "PRE_COPY_P2P"
