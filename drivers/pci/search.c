...
static enum pci_bus_isolation pcie_switch_isolated(struct pci_bus *bus)
{
	...
	/*
	 * Within a PCIe switch we have an internal bus that has the Upstream
	 * Port as the bridge and a set of Downstream Ports bridging to the
	 * egress ports.
	 *
	 ...
	 *
	 * Thus any permissive DSP spoils the whole bus.
	 */
 ...
 *
 * A bus is considered isolating if upstream directed transactions mastered
 * on the bus always continue to travel upstream. If the bus permits upstream
 * directed transactions to travel back downstream then it is a non-isolating
 * bus.
 *
 * We consider isolation on a bus by bus basis. If the bus will permit a
 * upstream directed transaction to on anything other than the IOMMU then the
 * bus is not isolating.
 *
 ...
 *
 * Broadly speaking this function evaluates the ACS settings in a PCIe switch to
 * determine if a PCIe switch is configured to have full isolation.
 *
 * Old PCI/PCI-X buses cannot have isolation due to their physical properties,
 * but they do have some aliasing properties that affect group creation.
 *
...
enum pci_bus_isolation pci_bus_isolated(struct pci_bus *bus)
{
	 ...
	 * within Linux to hold any bus numbers consumed by VF BDFs. Caller must
	...
	if (!pci_is_pcie(bridge)) {
		/*
		 * The bridge is not a PCIe bridge therefore this bus is
		 * PCI/PCI-X.
		 *
		 * PCI does not have anything like ACS. Any downstream device
		 * can bus master an address that any other downstream device
		 * can claim. No isolation is possible.
		 */
	...
	/*
	 * bus is the internal bus of a PCIe switch where DSP ACS rules apply.
	 */
	case PCI_EXP_TYPE_UPSTREAM:
	...
	case PCI_EXP_TYPE_PCI_BRIDGE:
		/*
		 * A PCIe bridge will use the subordinate bus number with a 0
		 * devfn as the RID in some cases. This causes all subordinate
		 * devfns to alias with 0, which is the same grouping as
		 * PCI_BUS_NON_ISOLATED. The RID of the bridge itself is only
		 * used by the bridge.
		 *
...
void pci_reachable_bus_set(struct pci_dev *start,
			   ...
			   bool (*reachable)(struct pci_dev *deva,
					     struct pci_dev *devb))
{
	struct pci_reachable_set new_devfns = {};
	...
	/* start is always part of the output set */
	...
	while (true) {
		...
		/*
		 * For each deva that hasn't been checked compare every device
		 * on the bus against it.
		 */
		again = false;
		for_each_set_bit(devfna, next_devfns.devfns, MAX_NR_DEVFNS) {
			...
			list_for_each_entry(devb, &bus->devices, bus_list) {
				...
				/*
				 * If the struct pci_dev for deva hasn't been
				 * found yet scan the rest of the list to get
				 * it.
				 */
				if (!deva) {
					deva = devb;
				...
				__set_bit(devb->devfn, new_devfns.devfns);
				again = true;
			}
		}
		...
		/*
		 * Every bit in todo adds a new deva to check, reloop the whole
		 * thing.
		 */
		for (i = 0; i != ARRAY_SIZE(devfns->devfns); i++) {
			devfns->devfns[i] |= new_devfns.devfns[i];
			next_devfns.devfns[i] = new_devfns.devfns[i];
			new_devfns.devfns[i] = 0;
		}
	}
	up_read(&pci_bus_sem);
}
EXPORT_SYMBOL_GPL(pci_reachable_bus_set);
