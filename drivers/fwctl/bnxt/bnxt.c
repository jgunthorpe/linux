			   ...
			   void *in, size_t in_len, size_t *out_len)
{
	...
	struct bnxt_fw_msg *rpc_in = in;
	int rc;

	if (!bnxtctl_validate_rpc(uctx, rpc_in))
		return ERR_PTR(-EPERM);

	rc = bnxt_send_msg(bnxt_aux_priv->edev, in);

	if (!rc) {
		printk(KERN_CRIT "failure calling bnxt_send_msg() rc = 0x%x\n", rc);
...
{
	struct bnxt_aux_priv *aux_priv = container_of(adev, struct bnxt_aux_priv, aux_dev);
...
{
	struct bnxtctl_dev *ctldev __free(bnxtctl) = auxiliary_get_drvdata(adev);
