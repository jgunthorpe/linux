...
/*
 * Exercise the FULL_CONT_RANGE_INV erratum case that needs three range invalidation commands.
 *
 ...
 * slices that CONT group. Extending the trailing range invalidation to cover it would round
 * its start below C - 1, outside the gathered range, so an exact third range invalidation for
 * [C, 2C) is required.
 */
static void test_armv8_three_range_inv(struct kunit *test)
{
	struct kunit_iommu_priv *priv = test->priv;
	struct pt_range top_range = pt_top_range(priv->common);
	...
	/*
	 * The KUnit page-table parameters select DBM for the SVA single range invalidation
	 * flow and TTBR1 for the paging-domain FULL_CONT_RANGE_INV flow.
	 */
...
static struct kunit_case iommu_test_cases[] = {
	...
	KUNIT_CASE_FMT(test_armv8_three_range_inv),
