...
static void __kunit_iotlb_erase_xa_range(struct kunit_iommu_inv_iotlb *iotlb,
					 bool leaf, unsigned long start_idx,
					 unsigned long last_idx,
					 unsigned int pgsz_lg2, u64 inval_start,
					 ...
					 struct kunit_iotlb_range *real_inval)
{
	...
	xa_for_each_range(xa, idx, entry, start_idx, last_idx) {
		...
		if (require_full && (start < inval_start || last > inval_last))
			continue;
...
static void kunit_iotlb_erase_xa_range(struct kunit_iommu_inv_iotlb *iotlb,
				       ...
				       struct kunit_iotlb_range *real_inval)
{
	__kunit_iotlb_erase_xa_range(iotlb, leaf, start_idx, last_idx, pgsz_lg2,
				     0, 0, false, real_inval);
}
...
static void
kunit_iotlb_erase_full_leaf_range(struct kunit_iommu_inv_iotlb *iotlb,
				  u64 start, u64 last, u64 leaf_sizes,
				  struct kunit_iotlb_range *real_inval)
{
	unsigned long start_idx;
...
struct kunit_armv8_inval {
	...
	bool ds;
	/* A single range invalidation must fully cover a CONT entry to invalidate it. */
	bool full_cont_range_inv;
	struct kunit_iotlb_range real_inval;
};
 ...
 */
static void
kunit_iotlb_armv8_assert_addr_align(struct kunit *test,
				  ...
				  struct kunit_armv8_inval *cmd)
{
	...
	kunit_iotlb_erase_range(iotlb, cmd->address, last,
				leaf_sizes & ~cont_sizes, table_sizes,
				&cmd->real_inval);
	kunit_iotlb_erase_full_leaf_range(iotlb, cmd->address, last, cont_sizes,
					  &cmd->real_inval);
}

static bool armv8_ttl_addr_aligned(u64 address, unsigned int tg,
				   unsigned int ttl)
{
...
struct kunit_armv8_range_inv {
	u64 start_tg;
	/* Normal integer, not encoded. 0 means no range invalidation. */
	u64 num;
	unsigned int scale;
};

static u64
armv8_range_inv_last_tg(const struct kunit_armv8_range_inv *range_inv)
{
...
static bool armv8_range_inv_encloses(const struct kunit_armv8_range_inv *a,
				     const struct kunit_armv8_range_inv *b)
{
	return a->start_tg <= b->start_tg &&
	       armv8_range_inv_last_tg(a) >= armv8_range_inv_last_tg(b);
}

/* Initialize the smallest range invalidation covering num_tg and ending at last_tg. */
static struct kunit_armv8_range_inv armv8_range_inv_init_end(u64 last_tg,
							     u64 num_tg)
{
	struct kunit_armv8_range_inv range_inv = {};

	if (!num_tg)
		return range_inv;

	range_inv.scale = fls64((num_tg - 1) / 32);
	range_inv.num = DIV_ROUND_UP_ULL(num_tg, 1ULL << range_inv.scale);
...
static void armv8_add_range_inv(struct kunit *test,
				struct kunit_iommu_inv_iotlb *iotlb,
				struct kunit_armv8_inval *cmd,
				const struct kunit_armv8_range_inv *range_inv,
				unsigned int ttl, unsigned int tg)
{
	KUNIT_ASSERT_NE(test, range_inv->num, 0);

	cmd->address = range_inv->start_tg << tg;
	cmd->tg = tg;
	cmd->ttl = ttl;
	cmd->num = range_inv->num - 1;
	cmd->scale = range_inv->scale;
	...
	/* A one-TG range invalidation without a TTL hint uses a non-range command. */
				      ...
				      unsigned int tg)
{
	...
	if (leaf_bitmap) {
		...
		if (table_bitmap) {
			int table_ttl = armv8_bitmap_to_level(table_bitmap) + 1;
...
enum kunit_armv8_range_inv_mode {
	KUNIT_ARMV8_RANGE_INV_NORMAL,
	KUNIT_ARMV8_RANGE_INV_SINGLE,
	KUNIT_ARMV8_RANGE_INV_CONT_ERRATA,
};

/* Number of contiguous entries grouped by CONT at an iommupt leaf level. */
static unsigned int armv8_cont_count_lg2(unsigned int tg, unsigned int level)
{
	if (tg == 12 && level <= 3)
...
static bool armv8_cont_errata(struct kunit_iommu_priv *priv,
			      struct iommu_iotlb_gather *gather,
			      const struct kunit_armv8_range_inv *first,
			      struct kunit_armv8_range_inv *trail,
			      struct kunit_armv8_inval *cmd, unsigned int ttl,
			      unsigned int tg)
{
	 ...
	 */
	while (leaf_levels) {
		struct kunit_armv8_range_inv cont = { .num = 1 };
		unsigned int level = fls(leaf_levels) - 1;
		struct kunit_armv8_range_inv new_trail;
		...
		cont_last_tg = armv8_range_inv_last_tg(&cont);
		if (cont.start_tg < range_start_tg ||
		    cont_last_tg > range_last_tg ||
		    armv8_range_inv_encloses(first, &cont) ||
		    armv8_range_inv_encloses(trail, &cont))
			continue;

		new_trail_num_tg = range_last_tg - cont.start_tg + 1;
		new_trail = armv8_range_inv_init_end(range_last_tg,
						     new_trail_num_tg);
		...
		armv8_add_range_inv(test, iotlb, cmd, &cont, ttl, tg);
		return false;
	}
	return false;
}
...
static void
kunit_iotlb_range_gather_armv8(struct kunit_iommu_priv *priv,
			       struct iommu_iotlb_gather *gather,
			       enum kunit_armv8_range_inv_mode range_inv_mode)
{
	struct kunit *test = priv->test;
	struct kunit_iommu_inv_iotlb *iotlb = priv->iotlb;
	...
	struct kunit_armv8_inval cmd = {
		...
		.full_cont_range_inv = range_inv_mode !=
				       KUNIT_ARMV8_RANGE_INV_NORMAL,
		.real_inval = IOTLB_RANGE_INIT,
	};
	struct kunit_armv8_range_inv first = {
	...
	if (range_inv_mode == KUNIT_ARMV8_RANGE_INV_SINGLE) {
		...
		trail = armv8_range_inv_init_end(
			last_tg, num_tg - (first.num << first.scale));
	}
	armv8_add_range_inv(test, iotlb, &cmd, &first, ttl, tgsz_lg2);

	if (trail.num) {
		if (range_inv_mode == KUNIT_ARMV8_RANGE_INV_CONT_ERRATA)
			armv8_cont_errata(priv, gather, &first, &trail, &cmd,
					  ttl, tgsz_lg2);
		armv8_add_range_inv(test, iotlb, &cmd, &trail, ttl, tgsz_lg2);
	}
out:
	...
	case KUNIT_IOMMU_MODEL_ARMV8:
		if (pt_feature(priv->common, PT_FEAT_ARMV8_DBM))
			kunit_iotlb_range_gather_armv8(
				priv, gather, KUNIT_ARMV8_RANGE_INV_SINGLE);
			...
			kunit_iotlb_range_gather_armv8(
				priv, gather, KUNIT_ARMV8_RANGE_INV_NORMAL);
		break;
