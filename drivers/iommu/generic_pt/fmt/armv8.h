...
enum {
	PT_MAX_TOP_LEVEL = 3,
	PT_GRANULE_LG2SZ = 12,
	PT_GRANULE_SIZE = ARMV8_GRANULE_SIZE,
};
...
enum {
	...
	/*
	 * S2TTB and TTB0 have an effective 64 byte minimum smallest alignment,
	 * but it may require a higher alignment to the size of the table.
	 */
...
static inline pt_oaddr_t armv8pt_oa(const struct pt_state *pts)
{
	...
	oa = oalog2_mul(FIELD_GET(ARMV8PT_FMT_OA48, entry), PT_GRANULE_LG2SZ);
...
static inline bool armv8pt_can_have_leaf(const struct pt_state *pts)
{
	 ...
	 */
	if ((PT_GRANULE_SIZE == SZ_4K && pts->level > 2) ||
	    (PT_GRANULE_SIZE == SZ_16K && pts->level > 1) ||
	    (PT_GRANULE_SIZE == SZ_64K && pts_feature(pts, PT_FEAT_ARMV8_LPA) && pts->level > 2) ||
	    (PT_GRANULE_SIZE == SZ_64K && !pts_feature(pts, PT_FEAT_ARMV8_LPA) && pts->level > 1))
		return false;
	return true;
}
			   ...
			   const struct pt_write_attrs *attrs)
{
	...
	/*
	 * On the last level the leaf is called a page and has the page/table
	 * bit set, on other levels it is called a block and has it clear.
	 */
	if (pts->level == 0)
					   ...
					   struct pt_write_attrs *attrs)
{
	attrs->descriptor_bits =
		pts->entry &
		(ARMV8PT_FMT_SH | ARMV8PT_FMT_AF | ARMV8PT_FMT_DBM |
		 ARMV8PT_FMT_UXN | ARMV8PT_FMT_PXN | ARMV8PT_FMT_ATTRINDX |
		 ARMV8PT_FMT_AP | ARMV8PT_FMT_nG | ARMV8PT_FMT_S2MEMATTR |
		 ARMV8PT_FMT_S2AP);
}
					 ...
					 const struct pt_iommu_armv8_cfg *cfg)
{
	...
	if (pt_feature(&armv8pt->common, PT_FEAT_ARMV8_S2) &&
	    PT_GRANULE_SIZE == SZ_4K && levels == 1)
		return -EINVAL;
...
static const struct pt_iommu_armv8_cfg armv8_kunit_fmt_cfgs[] = {
	[0] = { .common.features = BIT(PT_FEAT_ARMV8_DBM),
