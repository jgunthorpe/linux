...
enum {
	...
	PT_ITEM_WORD_SIZE = sizeof(u32),
	PT_MAX_TOP_LEVEL = 1,
	PT_GRANULE_LG2SZ = 12,
	PT_TABLEMEM_LG2SZ = 10,

	/* The lowest alignment is T0SZ=7 */
	PT_TOP_PHYS_MASK = GENMASK_ULL(31, 7),
};
...
{
	const u32 *entryp = pt_cur_table(pts, u32) + pts->index;
	unsigned int type;
	u32 entry;

	pts->entry = entry = READ_ONCE(*entryp);
			     ...
			     const struct pt_write_attrs *attrs)
{
	...
	if (oasz_lg2 == isz_lg2) {
		...
		end = entryp + log2_to_int(armv7s_pt_contig_count_lg2(pts));
...
static inline void armv7s_pt_change_leaf_oasz(struct pt_state *pts,
					      pt_oaddr_t oa,
					      unsigned int new_oasz_lg2)
{
	struct pt_write_attrs attrs;

	armv7s_pt_attr_from_entry(pts, &attrs);
	armv7s_pt_install_leaf_entry(pts, oa, new_oasz_lg2, &attrs);
}
#define pt_change_leaf_oasz armv7s_pt_change_leaf_oasz
