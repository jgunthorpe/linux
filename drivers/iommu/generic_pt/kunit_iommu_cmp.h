...
/* Defined by the format if it wants to test dynamic top */
...
static void kunit_iommu_cmp_iotlb_sync(struct iommu_domain *domain,
				       struct iommu_iotlb_gather *gather)
{
...
static const struct iommu_domain_ops pgtbl_ops = {
	...
	.iotlb_sync = &kunit_iommu_cmp_iotlb_sync,
};
...
static void test_cmp_high_va(struct kunit *test)
{
	...
	if (sizeof(unsigned long) == 4 && top_range.last_va >= U32_MAX) {
		...
		top_range.last_va = (u32)top_range.last_va;
	}

	do_cmp_map(test, top_range.last_va - (priv->smallest_pgsz - 1), 0,
		   priv->smallest_pgsz, IOMMU_READ | IOMMU_WRITE);
	compare_tables(test);
	do_cmp_unmap(test, top_range.last_va - (priv->smallest_pgsz - 1),
		     priv->smallest_pgsz);
	compare_tables(test);

	/*
	 * Map the biggest supported page at the very end of the IAS. This is
	 * the specific pattern that triggered bug d71fa842d33c in
	 * io-pgtable-arm (concatenated PGD index overflow). While generic_pt
	 * uses a single contiguous top allocation and is not affected by that
	 * specific bug, this exercises the top index computation for large
	 * entries near the end of the concatenated table.
	 */
	largest_pgsz = log2_to_int(priv->largest_pgsz_lg2);
	do_cmp_map(test, top_range.last_va - (largest_pgsz - 1), 0,
		   largest_pgsz, IOMMU_READ | IOMMU_WRITE);
	compare_tables(test);
}
...
static int pt_kunit_iommu_cmp_init(struct kunit *test)
{
	...
	cmp_priv = kunit_kzalloc(test, sizeof(*cmp_priv), GFP_KERNEL);
	if (!cmp_priv)
		return -ENOMEM;
	priv = &cmp_priv->fmt;
