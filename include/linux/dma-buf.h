 ...
 * the exporting using the DMA_BUF_EMAPPING_*() functions. Each mapping type
...
struct dma_buf_mapping_match {
	...
	union {
		...
		struct {
			...
			/*
			 * These p2p flags are used to support the old mechanism
			 * for p2p. If an exporting device knows it will put
			 * MMIO into the sgt then it should set
			 ...
			 * Newer code should migrate to use the P2P subsystem
			 * instead.
			 */
...
struct dma_buf_ops {
	...
	/*
	 * Called during attach, calls dma_buf_match_mapping() with the
	 * exporter's match list.
	 */
 ...
 * @map_type: The match that defines the mutally compatible mapping type to use
 *            for this attachment.
 *
